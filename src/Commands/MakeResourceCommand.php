...
final class MakeResourceCommand extends Command
{
    ...
    public function handle(Filesystem $files): int
    {
        ...
        if ($this->option('with-policy')) {
            ...
            $this->rewritePolicyToArqelDefaults($files, $modelBasename, $modelClass);
    ...
    /**
     * Laravel's default policy stub returns `false` for every
     * action — sensible for production but a "Hello World" UX
     * killer (the freshly-scaffolded Resource throws 403 before
     * the user can see anything). Rewrite the file with permissive
     * defaults plus TODOs that point the user at the right
     * tightening points.
     */
    protected function rewritePolicyToArqelDefaults(
        Filesystem $files,
        string $modelBasename,
        string $modelClass,
    ): void {
        $target = app_path("Policies/{$modelBasename}Policy.php");
...
/**
 * Auto-generated by `arqel:resource {$modelBasename} --with-policy`.
 *
 * Defaults to ALLOW everywhere so the Resource works out of the
 * box. Tighten each method as your authorization rules harden.
 */
final class {$modelBasename}Policy
{
    public function viewAny(User \$user): bool
    {
        // TODO: restrict to roles/abilities your app cares about.
        return true;
    }

    public function view(User \$user, {$modelBasename} \${$this->variableNameFor($modelBasename)}): bool
    {
        return true;
    }
    ...
    public function update(User \$user, {$modelBasename} \${$this->variableNameFor($modelBasename)}): bool
    {
        return true;
    }

    public function delete(User \$user, {$modelBasename} \${$this->variableNameFor($modelBasename)}): bool
    {
        return true;
    }

    public function restore(User \$user, {$modelBasename} \${$this->variableNameFor($modelBasename)}): bool
    {
        return true;
    }

    public function forceDelete(User \$user, {$modelBasename} \${$this->variableNameFor($modelBasename)}): bool
    {
        return true;
    }
}
PHP;
    ...
    protected function variableNameFor(string $modelBasename): string
    {
