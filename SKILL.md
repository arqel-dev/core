...
- **Command Palette backend (CMDPAL-001 — escopo reduzido):**
  ...
  - **CMDPAL-002 — Navigation + Theme providers (escopo reduzido)**: `Arqel\Core\CommandPalette\Providers\NavigationCommandProvider` (`final implements CommandProvider`, construtor toma `ResourceRegistry`, `provide()` itera `$registry->all()` e emite uma `Command` por Resource com id `nav:{slug}`, label `'Go to {pluralLabel}'`, url `/admin/{slug}`, category `'Navigation'`, icon do `getNavigationIcon()`; defensiva — broken Resource silently skipped). `ThemeCommandProvider` (`final implements CommandProvider`, 3 commands estáticos: `theme:light` / `theme:dark` / `theme:system` com category `'Settings'` + icons `sun`/`moon`/`monitor`). Auto-registo em `ArqelServiceProvider::packageBooted()` via `$registry->registerProvider(...)` para os 2. **`CreateCommandProvider` + `RecordSearchProvider` ainda deferidos** — precisam Policy authorization + Resource model traversal que é trabalho separado. **10 unit + 2 feature tests** novos (148 total core).
