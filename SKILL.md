...
- `ResourceController` (CORE-006) — 7 endpoints polimórficos (`index/create/store/show/edit/update/destroy`) sob `arqel.resources.{action}` + 1 endpoint bulk (`bulkAction`, `POST {resource}/bulk/{action}`, name `arqel.resources.bulk`) + 1 endpoint restore (`restore`, `POST {resource}/{id}/restore`, name `arqel.resources.restore`) + 1 endpoint custom-action (`rowAction`, `POST {resource}/actions/{action}/{id?}`, name `arqel.resources.action`). Resolve Resource pelo slug via `ResourceRegistry::findBySlug`, autoriza via `Gate::denies(viewAny|create|view|update|delete)`, materializa payload via `InertiaDataBuilder`, invoca lifecycle via `Resource::runCreate/runUpdate/runDelete`. Validation via `FieldRulesExtractor` (carregado via Reflection — sem hard dep em `arqel-dev/form`). **A extração de regras é record-aware (#198):** `extractRules`/`validated` threadam o registro carregado (no update; `null` no create) para `effectiveFields($record)`, de modo que um field `required` encerrado num layout escondido para **aquele** registro (`Section::visibleIf`/`canSee`) **não** é validado — espelhando os caminhos de render e write-prune, que também o omitem (completa a perna de validação do #115). O guard fail-closed (extrator ausente ⇒ fallback permissivo; extrator quebrado ⇒ 500) permanece intacto.
  - **Enforcement de autorização no write path (#102/#115):** antes do mass-assign, `store`/`update` podam o input contra os fields da Resource — um field cujo `canBeEditedBy($user, $record)` **nega** o usuário tem o seu valor descartado (não persiste), fechando um bypass de mass-assignment em que um field `canEdit(false)`/`canSee(false)` (ou um bloco de layout `canSee`/`visibleIf` invisível) ainda assim era gravado se chegasse no payload. Como `HasAuthorization` encadeia `canBeSeenBy` em `canBeEditedBy`, podar por `canBeEditedBy` cobre ambos. A visibilidade no nível de layout é reflectida porque o `fields` payload já vem podado de `Form::getFields($record)` (#115, ver SKILL do `arqel-dev/form`).
  - **Modificadores não-persistentes (#127):** valores de fields `dehydrated(false)` (o contrato documentado "não persista"), `disabled()` e `readonly()` são igualmente descartados antes do mass-assign — são display-only/computed, não input que o framework deva aceitar. Cada oracle tem default no estado persistente (`readonly=false`, `disabled=false`, `dehydrated=true`), então um field comum não é afectado.
  - **Restore de soft-delete (#244):** rota `POST {resource}/{id}/restore` (name `arqel.resources.restore`) + método `restore()`. `Actions::restore()` serializa essa URL (via `Action::resolveStockUrl`) mas nenhuma rota a respaldava → 404; agora existe, espelhando os constraints/middleware de `destroy`. O método carrega o registro **com trashed** (`findWithTrashedOrFail` — o scope `SoftDeletes` o esconderia), é **tenant-scoped**, autoriza a ability `restore` (convenção Laravel), chama `$record->restore()` quando disponível e redirect para o index com flash de sucesso. (Custom actions despachadas por `rowAction` ainda carregam non-trashed — restore-shaped custom action sobre row trashed fica fora de escopo do #244.)
  - **Bulk dispatch (#48/#69):** `bulkAction` resolve os records via `whereIn($model->getKeyName(), $ids)` — usa a **chave primária real** do model, não `'id'` hardcoded (#69), então models com PK custom (`uuid`, `slug`, ...) funcionam. **Qualquer** bulk action é despachada via `BulkAction::execute(Collection)` (não só `delete`); um bulk sem callback recai no delete em massa só quando `action === 'delete'`. O `Action::resolveStockUrl()` emite uma URL stock convencional (`POST /admin/{slug}/bulk/{name}`) para toda bulk action mesmo sem callback, então o frontend nunca aponta para uma rota inexistente (#48). As rotas standalone `arqel.actions.*` foram **removidas** — todo despacho de action passa pelas rotas de resource de core (#174, ver SKILL do `arqel-dev/actions`).
  - **Custom row/header/toolbar action dispatch (#231):** `rowAction` (`POST {resource}/actions/{action}/{id?}`, name `arqel.resources.action`) despacha uma action customizada com `->action(Closure)` (sem `->url()` explícito) que antes não tinha caminho HTTP — as rotas standalone `arqel.actions.*` foram removidas (#174) e `resolveStockUrl()` só cobria os verbos stock + bulk, então o frontend recaía na rota morta `/arqel-dev/actions/{name}` → 404. Espelhando `bulkAction` (#48): resolve a action por nome nas coleções `actions`/`headerActions`/`toolbarActions` (duck-typed, sem hard dep em `arqel-dev/actions`), **autoriza em duas camadas** — o Gate de resource (`update` quando há record-alvo, `viewAny` para toolbar sem record) **e** o `canBeExecutedBy($user, $record)` da própria action (ACTIONS-005, 403 em negação) — valida o payload do form modal quando presente, e roda `execute($record, $data)` flashando success/failure. Carrega o mesmo stack de middleware (web + auth + tenant) das demais rotas de resource. O `Action::resolveStockUrl()` emite a URL correspondente para uma custom action com callback, então o frontend tem sempre um endpoint real.
- `HandleArqelInertiaRequests` middleware (CORE-007) — estende `Inertia\Middleware`. Shared props: `auth.user` (`only(['id','name','email'])`), `auth.can` (delegated to `AbilityRegistry::resolveForUser` quando `arqel-dev/auth` está bound), `panel`, `tenant`, `flash` (success/error/info/warning closures), `translations` (`arqel::*`), `arqel.version`. **`panel.navigation`** é populado por `buildNavigation()` que itera os Resources do panel actual e devolve `{ label, url, icon, group, sort, active }` por entry — derivando label de `getPluralLabel()`, url de `/{panel.path}/{slug}`, icon de `getNavigationIcon()`, group de `getNavigationGroup()`, sort de `getNavigationSort()`, active comparando o path actual da request com a url do entry. A ordem dos itens emitidos respeita `Panel::navigationGroups([...])`: os grupos aparecem na ordem dessa lista explícita; grupos ausentes da lista (e o bucket sem grupo) vêm a seguir, ordenados pelo `navigationSort` mínimo de cada grupo (fallback que preserva o comportamento por-item quando `navigationGroups()` não é definido); dentro de cada grupo os itens mantêm ordenação por `navigationSort`. Como o cliente (`Sidebar.groupItems()`) agrupa por primeira-ocorrência, emitir a lista já agrupada+ordenada no servidor é suficiente para a ordem dos grupos ficar correcta. Resources que rebentem em qualquer dos getters são silently skippados. **Autorização (#118):** o `$request->user()` é encadeado de `share()` → `currentPanel()` → `buildNavigation()`, e cada item é omitido quando a Policy `viewAny` do model da Resource **nega** o usuário — via o guard compartilhado `Support\ResourceAuthorization::viewAnyDenied()` (mesma checagem reusada pelo command palette, #129), espelhando `ResourceController::authorize()`: o gate só é consultado quando existe um named gate **ou** uma Policy (`Gate::has('viewAny') || Gate::getPolicyFor($modelClass)`); sem nenhum dos dois (scaffold mode) tudo segue listado, e Resources sem `$model` declarado nunca são ocultadas.
...
- **Locale switching (#105):** `LocaleController` persiste a escolha de idioma num cookie `arqel_locale` (`Cookie::forever`), e o `SetLocaleMiddleware` lê esse cookie (tier-2 cross-session) para aplicar o locale em requests subsequentes. Antes a troca só durava o request atual; agora a preferência sobrevive a navegações e novas sessões sem precisar de auth/DB.
